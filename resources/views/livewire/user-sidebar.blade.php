<aside class="hidden shrink-0 ltr:mr-8 rtl:ml-8 xl:block xl:w-80">
    <div class="overflow-hidden rounded border border-border-200 bg-light">
        <ul class="py-7">
            ...
            <li class="py-1"><a class="block border-l-4 border-transparent py-2 px-10 font-semibold text-heading transition-colors hover:text-accent focus:text-accent {{ $current_route == 'frontend.notifications.index' ? '!border-accent text-accent' : '' }}" wire:navigate href="{{ route('frontend.notifications.index') }}">{{ __trans('Notifications') }}</a></li>
            @endif
