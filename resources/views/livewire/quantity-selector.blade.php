        ...
        <div>
            <template x-if="'{{ $layout }}' === 'cart'">
                <div class="flex overflow-hidden flex-col-reverse items-center w-8 h-24 bg-accent text-heading rounded-full text-accent-contrast">
                    <button @click="if(quantity > 0) { quantity--; $wire.decrement(); }" 
                    ...
                    <button @click="quantity++; $wire.increment()" 
                            class="cursor-pointer p-2 transition-colors duration-200 hover:bg-accent-hover focus:outline-0 px-5">
            ...
            <template x-if="'{{ $layout }}' !== 'cart' && '{{ $layout }}' !== 'large'">
                <div class="flex overflow-hidden order-5 sm:order-4 w-9 sm:w-24 h-24 sm:h-10 bg-accent text-light rounded-full flex-col-reverse sm:flex-row absolute sm:relative bottom-0 sm:bottom-auto ltr:right-0 rtl:left-0 ltr:sm:right-auto ltr:sm:left-auto text-accent-contrast">
                    <button @click="if(quantity > 0) { quantity--; $wire.decrement(); }" 
