<div>
    ...
    <button wire:click="remind" type="button" 
            ...
            border {{ $isReminded ? 'border-active' : 'border-gray-300' }} p-1" title="{{ __trans('Remind me when stock is available') }}">
    ...
    <div x-data="{ isDrawerOpen: @entangle('isDrawerOpen') }" 
         x-show="isDrawerOpen"
         class="fixed inset-0 z-50 bg-gray-100 bg-opacity-50 transition-opacity duration-300 flex justify-end"
         x-cloak>
        <!-- Drawer content -->
        <div class="h-full w-80 bg-white shadow-lg transition-transform transform duration-300 ease-in-out"
            ...
            <div class="p-6">
                <h2 class="text-lg font-semibold mb-6 text-gray-700">{{ __trans('Enter Your Email') }}</h2>
                
                <form wire:submit.prevent="submitEmail" class="space-y-4">
                    <!-- Email Input -->
                    <div>
                        <label for="email" class="block text-sm font-medium text-gray-700 mb-1">{{ __trans('Enter Address') }}</label>
